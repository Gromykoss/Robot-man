...
## 2026-08-14 — Конный мини-фильм: целиком сгенерированный вестерн (эталон «волк»), 6 сцен, финальный цикл

Сессия Hermes (default) по конному ролику. Главный итог: переход от «вставки персонажей в сырые исходники» к **полностью сгенерированному единому мини-фильму** (эталон уровня — ролик «волк» пользователя). Погоня с ковбоями+индейцами отменена → заменена приёмом без врагов (табун / река / буря).

- **Сценарий (6 сцен, единый дальний масштаб, без врагов):** 1) подготовка (со спины, контражур) → 2) выезд галопом → 3) герой в табуне диких лошадей → 4) переправа через реку → 5) буря (силуэт) → 6) уход к горам на закате.
- **Герой** = сын Сергея. Портретного фото нет → правило: в движении/неоднозначных ракурсах лицо **скрывать масштабом и светом** (дальний план, со спины, профиль, контражур). Сходство держится одеждой (бордовая футболка «TRUST YOUR INSTINCT», чёрная кепка, бежевые шорты, кроссовки) + лошадью (светлая + красная попона).
- **Ключевое открытие (face-lock):** Grok Imagine (`grok-imagine-image-quality`) **принципиально не умеет** жёсткий 100% face-lock реального человека (нет identity-embedding/FaceID). «Сохрани лицо 100%» даёт 30-95% максимум, лицо «плывёт» в местный типаж («киргиза»). Решение режиссёрское: скрывать лицо, а не перегенерировать в лоб. Близкие «портретные» кадры в таком фильме — запрещены.
- **Баг «замирание» H3 i2va:** первые ~1.5с статичны («герой стоял, потом скачет»). По скриншотам НЕ видно (vision видит «замороженный галоп»). Решение: подрезать первые 1.5с динамичных сцен (`ffmpeg -ss 1.5`).
- **Баг «резкие блики» на стыках:** жёсткий cut между сценами с «горячими» пересвеченными облаками даёт вспышку. Решение: crossfade (`xfade=fade=1с`) на всех 5 стыках + лёгкое `eq=...` приглушение.
- **Грабля xfade offset:** наивная формула разъезжается на 4-м+ стыке. Правильно: `offset_i = offset_{i-1} + dur_i - FADE`; итог = сумма − fade×(N−1).
- **Результат:** `~/Downloads/вестерн_финал_crossfade.mp4` — 52.75с, 768×1024 (вертикаль), 6 сцен по ~10с, плавные переходы, вестерн-музыка (w26_real.mp3). Переходы одобрены Сергеем («норм»).
- **Документация обновлена:** `Видеомонтаж - база знаний.md` (новый раздел 8.5 + 5 грабель), скилл `ai-video-montage` (раздел 10 + Pitfalls).
- **Следующий этап (отложен):** кадры разной длины (у H3 `duration` 4-15с под смысл) + полноценная сюжетная арка героя. Пока все 6 сцен по 10с — это «набросок».
